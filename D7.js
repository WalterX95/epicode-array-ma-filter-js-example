...
function cutString(string,string2) {
     string.join('');
     string2.join('');
...
const arrayElement = [];
 ...
 function randomElement() {
      for(let i = 20; i > arrayElement.length; i--) {
           let randomNumber = Math.floor((Math.random() * 100) + 1);
           arrayElement.push(randomNumber);
      }
      return arrayElement;
 }
...
const sumDataForE = arrayElement.forEach((element) => {
    element = element + element;
    return element;
 });

 console.log(sumDataForE);

/* ESERCIZIO 5 (reduce)
  Scrivi una funzione per sommare i numeri contenuti in un array
*/

const data = arrayElement.reduce((element) => {
      element = element + element;
      return element;
});
...
const newArray = arrayElement.map((element,n) => {
     return element + n;
});
...
const newArrNumb = arrayString.map((element) => {
...
*/
/*function indexMovie(indexMovie) {
    indexMovie = movies[0];
    indexMovie.forEach(function(element) {
       return element.length;
    });
}
...
const onlyTitle = movies.map((element) => {
